...
Distance_km Varchar(30) NOT NULL Check (Distance_km > 0))
...
Class Varchar(30) check(Class IN('Sleeper', 'AC1', 'AC2', 'AC3')),
Fare Int Check(Fare >= 0),
Status Varchar(30) Check(Status IN('Confirmed', 'Waiting', 'Cancelled')))
...
Alter Column PassengerId INt NOT NULL

Alter table Passengers
Add Constraint pk_passengers Primary Key (PassengerId)

Alter Table Reservations
Add Constraint fk_reservations Foreign Key (TrainID) References Trains(TrainID)

Alter Table Reservations
Add Constraint fk_reservations1 Foreign Key (PassengerId) References Passengers(PassengerId)
...
Select Count(Resid) AS Total_Reservations from Reservations

--4.Count how many reservations are in 'Confirmed' status.

Select Count(Resid) AS Confirmed_Reservation from Reservations Where Status='Confirmed'; 

--5.Show the average fare paid for each train.

Select TrainID, AVG(Fare) As Average_Fair_Paid from Reservations Group By TrainID

--6  List reservations ordered by fare in descending order.

Select ResId, Fare from Reservations Order BY Fare DESC

--7  Find passengers who booked AC classes (AC2, AC3).

Select * from Passengers Where PassengerId IN (select PassengerId from Reservations where Class in ('AC2', 'AC3'));

--8  Get total revenue collected per train (only Confirmed reservations).

Select TrainID, SUM(Fare) AS Total_Revenue From Reservations Group By TrainID
...
Select * from Passengers
SELECT Pname AS 'Passengers from Delhi and Mumbai'
FROM Passengers
WHERE City = 'Delhi'
OR City = 'Mumbai'
...
--84. Find the train with the maximum distance_km.

Select TrainName from Trains Group By MAX(Distance_km)
